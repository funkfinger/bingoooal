...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    if (!board_id || typeof locked !== 'boolean') {
      return new Response(JSON.stringify({ success: false, error: 'Board ID and locked status are required' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    if (board.user_id !== session.user.id) {
      return new Response(JSON.stringify({ success: false, error: 'Unauthorized to modify this board' }), {
        status: 403,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    if (updateError) {
      ...
      return new Response(JSON.stringify({ success: false, error: 'Failed to update board lock status' }), {
        status: 500,
        headers: { 'Content-Type': 'application/json' },
      });
    }
